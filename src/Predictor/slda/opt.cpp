    ...
    for (d = 0; d < ss->num_docs; d ++)
    {
        ...
        for (l = 0; l < model->num_classes-1; l ++)
        {
            ...
            for (k = 0; k < model->num_topics; k ++)
            {
                ...
                {
                    idx = map_idx(k, j, model->num_topics);
                    //TODO Moja modyfikacja
                    double tmp1 = model->eta[l][k];
                    double tmp2 = model->eta[l][j];
                    a2 += tmp1 == 0 || tmp2 == 0 ? 0 : tmp1 * ss->z_bar[d].z_bar_var[idx] * tmp2;
                    //a2 += model->eta[l][k] * ss->z_bar[d].z_bar_var[idx] * model->eta[l][j];
                    eta_aux[k] += tmp2 == 0 ? 0 : ss->z_bar[d].z_bar_var[idx] * tmp2;
                }
            }
            a2 = 1.0 + 0.5 * a2;
            t = log_sum(t, a1 + log(a2));
