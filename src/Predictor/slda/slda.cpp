...
slda::slda()
{
	logger = lgr::Logger::getLogger("Main.Predictor.slda");
	TRACE(logger, "Creating SLDA");
    //ctor
    alpha = 1.0;
    num_topics = 0;
    num_classes = 0;
...
void slda::corpus_initialize_ss(suffstats* ss, corpus* c)
{
    ...
    for (k = 0; k < num_topics; k++)
    {
        for (i = 0; i < NUM_INIT; i++)
        {
            ...
            TRACE(logger, "initialized with document " << d);
            document * doc = c->docs[d];
                ...
                const char * start, const char * directory)
{
    ...
    {
        //final inference
        if ((d % 100) == 0) TRACE(logger, "final e step document " << d);
...
void slda::mle(suffstats * ss, int eta_update, const settings * setting)
{
	...
	do
	{
		...
		f = -s->f;
		if ((opt_iter-1) % 10 == 0)
			INFO(logger, "Step: " << opt_iter-1 << " -> f: " << f);
...
void slda::infer_only(corpus * c, const settings * setting, const char * directory)
{
    ...
    // -- moje
    int *catCorrect = new int[num_classes];
    int* catImgs = new int[num_classes];
    ...
    int max_length = c->max_corpus_length();
    double * score_all;								// new table of score per image
    score_all = new double [num_classes];			// allocation memory
    int * results;									// new table to save results
    results = new int [num_classes];				// to be sorted
    ...
    {
        if ((d % 100) == 0)
        	TRACE(logger, "document " << d);

        document * doc = c->docs[d];
        TRACE(logger, "doc label: "<<doc->label );
        TRACE(logger, "doc total: "<<doc->total );
        TRACE(logger, "doc length: "<<doc->length );
        ...
        for (i = 0; i < num_classes; i++) score_all[i] = 0.0; // clean table before calculations

        for (i = 0; i < num_classes-1; i ++)   // num_classes - 1
        {
            score = 0.0;
            for (k = 0; k < num_topics; k ++)
            {
            ...
            score_all[i] = score;		// actualize score in table
        ...
        // just print table with scores
        // for( i = 0; i < num_classes; i++) printf("%.3f\t", score_all[i]);

    	double min = DBL_MAX;			// minimum, firstly set up as highest number

        for( i = 0; i < num_classes; i++ ) results[i] = 0;       // clean results table, that fetch number of indexes in best order

        // Normalization
        for( i = 0; i < num_classes; i++ ){						 // find global minimum
        	if( min > score_all[i] ){
        		min = score_all[i];
        	}
        }
        ...
        for( i = 0; i < num_classes; i++ ){
        	score_all[i] -= min;								  			// move offset, afterwards all values are >= 0
        }

        // for( i = 0; i < num_classes; i++ ) printf("%.3f\t", score_all[i]); 	// shows score after moving offset

        double max = -DBL_MAX;												//  setup max, firstly as very low
        for( int z = 0; z < num_classes; z++){
        	...
        	score_all[results[z]] = -score_all[results[z]];
        	max = -DBL_MAX;
       }

        double max_score = score_all[results[0]];
        double sum_score = 0;
        double local_score;

        for( i = 0; i< num_classes - 1; i++){
            sum_score += score_all[results[i]];
        }


    std::vector<std::string> cats = { "Forest", "InsideCity", "OpenCountry", "Mountain", "Highway", "TallBuilding", "Coast", "Street"};
    if( showResults ) {
        for( i = 0; i < num_classes - 1; i++) {

            local_score = score_all[results[i]]/sum_score*100;
            // do sth like histogtram streching
            local_score = (local_score - score_all[results[num_classes-2]]/sum_score*100) * 99 / (score_all[results[0]]/sum_score*100 - score_all[results[num_classes-2]]/sum_score*100) + 1;

            std::cout << cats.at(results[i]) << " " << int( local_score ) << "%" <<  " > ";
        }
        std::cout << std::endl;
	}
    ...
    if( showAverage ) {
    	...
    	for(int i = 0; i < num_classes; i++) {
    		printf("%i = %.3f\n", i, (float)catCorrect[i] / catImgs[i] );
    	}
    }
    ...
    delete [] phi_m;
    delete [] score_all;	// delete score per_image table - free memory
    delete [] results;		// delete results table
}
