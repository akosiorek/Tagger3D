...
The capitalized parameters are the essential parambers of the
...
#ifndef USE_BLAS
#define USE_BLAS    0             // 1: use BLAS for fast L2 distance calculation
...
static inline void timer_reset (timer_t *t) {
    *t = time(NULL);
}

static inline float timer_elapsed (timer_t *t) {
    return time(NULL) - *t;
}
