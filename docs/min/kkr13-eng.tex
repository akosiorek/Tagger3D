...
\abstract{ 
 ...
 This paper introduces a Bag of Words (BoW) semantic object classification 
framework based on RGBD data registered by a Microsoft Kinect camera. We 
introduce a BoW image processing 
pipeline, describe it's steps in detail (region detection, feature 
extraction and vector quantization) and we outline the most successful 
algorithms. Algorithms with high performance potential were identified and 
evaluated. 
Salient region detection is done by SIFT, ISS3D and Uniform Sampling. 
Description is performed by FPFH, PFH and PFHRGB. Vector quantization was 
carried out by KMeans. An SVM with the RBF kernel trained with an 
All-vs-All method was used as a classifier. We achieved the accuracy of 65.22\% 
on 8 
categories of the Berkely 3D Object Dataset and 62.3\% on a 10 category dataset 
compiled by Zhang.
