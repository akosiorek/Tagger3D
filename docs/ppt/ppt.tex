...
\begin{frame}{Bag of Words w obrazach}	
	...
	\begin{enumerate}
	 ...
	 \begin{itemize}
	  \item współrzędne (x, y) oraz skala obrazka (SIFT, FAST)
	 \end{itemize}

	 \item Opisanie otoczenia wykrytych punktów
	 \begin{itemize}
 	  \item opis jednoznaczny, zazwyczaj poprzez przestrzenny histogram gradientów (SIFT, SURF, HOG)
	 \end{itemize}
	 \item Budowanie słownika i kwantyzacja 
	 \begin{itemize}
	  \item punkty układają się w obszary, które można wykryć - klasteryzacja (KMeans, GMM, Fisher Vector)
	 \end{itemize}
	 \item Klasyfikacja
	 \begin{itemize}
	  \item klasyfikator uczący się - (SVM, PGM, Boosting)
	 \end{itemize}
	\end{enumerate}	
\end{frame}{}
...
\begin{frame}{Projekt aplikacji}
	 ...
	 \begin{column}{6cm}
	  \begin{figure}
	    \includegraphics[width=1.1\textwidth]{../figs/class}	   
	  \end{figure}
	 ...
	 \begin{column}{6cm}
	  \begin{figure}
	  \includegraphics[width=0.6\textwidth]{../figs/strategy}	
	  \end{figure}
...
\begin{frame}{Berkely 3D Object Dataset}
	...
	\begin{columns}
	 ...
	 \begin{figure}[!ht]
	  ...
	  \end{figure}
	  \centering \footnotesize Obiekty z B3DO. Zdjęcia w oryginalnej jakości. a) kubek b) butelka c) książka d) klawiatura
	 \end{column}
...
\begin{frame}{University of Tokyo Dataset}
	...
	\end{figure}
	\centering \footnotesize Baza danych Tokyo: a) kosz b) rower c) pudełko d) wiadro e) wózek f) lodówka g) notebook h) drukarka i) skaner j) scena
	\normalsize
